...
if i == N
    ...
    while ~converged && iters < maxIter
        ...
        [psi, phi, Dpsi] = evaluateRationals(mu, delta, t, i, N, tol);
    ...
    while ~converged && iters < maxIter
        ...
        % Compute some helper values
        D = delta(i+1) - mu;
        [psi, phi, Dpsi, Dphi] = evaluateRationals(mu, delta, t, i, N, tol);
        if isinf(Dpsi) || isinf(Dphi)
            keyboard
            % error('Dpsi/Dphi computation failed.');
        end
...
function [psi, phi, Dpsi, Dphi] = evaluateRationals(x, delta, u, i, N, tol)
iL = 1:i;
iU = (i+1):N;
...
if isinf(psi), psi = 1/tol; end
if isinf(Dpsi), Dpsi = 1/tol; end
if isinf(phi), phi = 1/tol; end
if isinf(Dphi), Dphi = 1/tol; end